...
create table if not exists public.deals (
  ...
  icp_evidence  text,
  icp_confidence smallint check (icp_confidence is null or icp_confidence between 0 and 2),
  icp_model     text,
...
-- ================================================================
-- STORY 054: DISPARO MANUAL SEGURO DE AUTOMACOES DE E-MAIL
-- ================================================================
...
create trigger email_automation_enrollments_updated_at before update on public.email_automation_enrollments
  for each row execute function public.set_updated_at();
drop trigger if exists email_automation_dispatches_updated_at on public.email_automation_dispatches;
create trigger email_automation_dispatches_updated_at before update on public.email_automation_dispatches
  for each row execute function public.set_updated_at();

create or replace function public.claim_email_automation_dispatch(
  p_automation_id uuid, p_automation_version integer, p_enrollment_id bigint,
  p_node_id text, p_step_index integer, p_recipient_email text, p_subject text,
  p_html text, p_idempotency_key text, p_actor text, p_daily_cap integer
)
...
begin
  if p_daily_cap is null or p_daily_cap not between 1 and 250 then raise exception 'invalid_daily_cap'; end if;
  if nullif(trim(p_subject), '') is null or nullif(trim(p_html), '') is null then raise exception 'empty_email_snapshot'; end if;
  perform pg_advisory_xact_lock(20260910,
    (timezone('America/Sao_Paulo', now())::date - date '2020-01-01')::integer);
  if not exists (select 1 from public.email_automations where id = p_automation_id and version = p_automation_version and status = 'validated') then
    raise exception 'automation_not_validated';
  end if;
  select * into v_enrollment from public.email_automation_enrollments where id = p_enrollment_id for update;
  if not found or v_enrollment.automation_id <> p_automation_id or v_enrollment.automation_version <> p_automation_version
     or v_enrollment.status <> 'pending' or v_enrollment.next_step <> p_step_index or v_enrollment.next_due_at > now()
     or v_enrollment.recipient_email <> lower(trim(p_recipient_email)) then return; end if;
  v_day_start := date_trunc('day', timezone('America/Sao_Paulo', now())) at time zone 'America/Sao_Paulo';
  select count(*)::integer into v_legacy_count from public.activities
    where type = 'email_sent' and created_at >= v_day_start and nullif(metadata->>'dispatch_id', '') is null;
  select count(*)::integer into v_claimed_count from public.email_automation_dispatches
    where claimed_at >= v_day_start and status in ('claimed', 'sent', 'uncertain');
  if v_legacy_count + v_claimed_count >= p_daily_cap then return; end if;
  if exists (select 1 from public.email_automation_dispatches where enrollment_id = p_enrollment_id
      and claimed_at >= v_day_start and status in ('claimed', 'sent', 'uncertain')) then return; end if;
  insert into public.email_automation_dispatches (
    enrollment_id, automation_id, automation_version, node_id, step_index, idempotency_key,
    recipient_email, subject, html_snapshot, created_by
  ) values (
    p_enrollment_id, p_automation_id, p_automation_version, p_node_id, p_step_index, p_idempotency_key,
    lower(trim(p_recipient_email)), p_subject, p_html, coalesce(p_actor, '')
  ) on conflict do nothing returning id into v_dispatch_id;
  if v_dispatch_id is null then return; end if;
  return query select v_dispatch_id, p_idempotency_key;
end;
$$;

revoke all on function public.claim_email_automation_dispatch(uuid, integer, bigint, text, integer, text, text, text, text, text, integer)
  from public, anon, authenticated;
grant execute on function public.claim_email_automation_dispatch(uuid, integer, bigint, text, integer, text, text, text, text, text, integer)
  to service_role;

create or replace function public.complete_email_automation_dispatch(
  p_dispatch_id bigint, p_provider_message_id text, p_next_step integer,
  p_next_due_at timestamptz, p_completed boolean
)
...
declare
  ...
  v_enrollment public.email_automation_enrollments%rowtype;
begin
  select * into v_dispatch from public.email_automation_dispatches where id = p_dispatch_id for update;
  ...
  select * into v_enrollment from public.email_automation_enrollments where id = v_dispatch.enrollment_id for update;
  if not found or v_enrollment.next_step <> v_dispatch.step_index then raise exception 'enrollment_step_conflict'; end if;
  update public.email_automation_dispatches set status = 'sent', provider_message_id = trim(p_provider_message_id), sent_at = now(), error = null
    where id = p_dispatch_id;
  update public.email_automation_enrollments set next_step = p_next_step, next_due_at = coalesce(p_next_due_at, now()),
    last_sent_at = now(), status = case when p_completed then 'completed' else 'pending' end, stop_reason = null
    where id = v_dispatch.enrollment_id;
  insert into public.activities (deal_id, contact_id, type, description, metadata, created_at)
  values (v_enrollment.deal_id, v_enrollment.contact_id, 'email_sent',
    'E-mail enviado para ' || v_dispatch.recipient_email || ': ' || v_dispatch.subject,
    jsonb_build_object('provider', 'brevo', 'message_id', trim(p_provider_message_id),
      'automation_id', v_dispatch.automation_id::text, 'automation_version', v_dispatch.automation_version,
      'node_id', v_dispatch.node_id, 'step_index', v_dispatch.step_index,
      'dispatch_id', v_dispatch.id::text, 'mode', 'manual_crm'), now());
  return true;
end;
$$;
