...
import { ehRespostaAutomatica } from "./autoresponder.mjs";
...
export function classifyInboundResponse(content: string): ResponseType {
  ...
  // Lista unica de resposta automatica (Story 067): src/lib/autoresponder.mjs. O historico
  // de cada padrao (deal de origem) mora la; a cadencia (uazapi-followup-batch) usa a mesma.
  if (ehRespostaAutomatica(content)) return "bot";
