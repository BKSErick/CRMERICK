...
import { aiComplete, aiCompleteDetailed, describeFailures } from "@/lib/aiComplete";
...
${messageEvidence || "Nenhuma mensagem registrada."}${signalContext}`;
    ...
    // Mesma cascata do resto do CRM (src/lib/aiProviders.mjs): modelos vindos do catálogo
    // vivo, sem nome fixo aqui. Antes esta rota mantinha uma cópia própria da lista, que
    // envelheceu apontando para modelos já descontinuados.
    const { result, failures } = await aiCompleteDetailed(systemPrompt, userPrompt, {
      signal: request.signal,
    });

    if (!result) {
      return NextResponse.json(
        { ok: false, error: describeFailures(failures), detail: failures },
        { status: 502 }
      );
    }
